...
int stega_encrypt(FILE *fp1, FILE *fp2, FILE *fp3)
{
...
int Encoding(char *argv_2,char *argv_3,char *argv_4)
{
	...
	printf("*** Password Embedded ***\n");

	//Encryption for Message
	size_encrypt(size_txt,fp1,fp3);
	count=stega_encrypt(fp1,fp2,fp3);
        double mse = ((double)count)/((double)(size_image*8/3));
